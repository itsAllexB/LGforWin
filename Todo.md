...
Ideas / next steps after **v1.0**. (Also summarized in the README roadmap.)
...
- [ ] **Installer** — an Inno Setup `setup.exe` (Start-menu shortcut, uninstall entry) as an
      alternative to the portable zip.
- [ ] Make the release tag and the in-app version read identically (`v1.0` vs `v1.0.0`).

## Features
- [ ] **SSDP auto-discovery** — find LG TVs on the LAN so users don't have to type IPs.
